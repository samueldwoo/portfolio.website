...
| `golf_lies.py` | **the SECOND stroke**: from every lie the tee can leave you in, can the cup still be won? Player-model-free. Solvability only ever asked about the tee. Uses `sweep(starts=…)` to batch hundreds of lies per call, escalates mid→full grid, and plays untested onward lies before calling anything terminal. Ordinary holes ~1 min, trap holes 10–20 min — shard with `--only` |
